            ...
            <div className="nav w-100 fixed-top top-0 start-0 bg-light shadow-sm">
                <nav className="container navbar navbar-expand-lg py-2">
                    <Link to='/' className='navbar-brand'>
                        <h3 style={{ color: primaryColor, margin: 0 }}>MENSCO</h3>
                    </Link>

                    <div className="d-lg-none mobile-icon d-flex align-items-center gap-3">
                        <Link to='/'><i className="bi bi-search fs-5"></i></Link>
                        <Link to='/'><i className="bi bi-person fs-5"></i></Link>
                        <Link to='/wishlist' className='position-relative'>
                            <i className="bi bi-heart fs-5"></i>
                            <span className='position-absolute top-0 start-100 translate-middle cart-count rounded-pill'>{wishlistCount ?? 0}</span>
                        </Link>
                        <Link to='/cart' className='position-relative'>
                            <i className="bi bi-bag fs-5"></i>
                            <span className='position-absolute top-0 start-100 translate-middle cart-count rounded-pill'>{cartCount ?? 0}</span>
                        </Link>
                    </div>
                    ...
                    <div className="collapse navbar-collapse justify-content-center" id="navbarNav">
                        ...
                        <ul className="navbar-nav ms-auto d-none d-lg-flex align-items-center gap-3">
                            <li><Link to='/' data-bs-toggle='modal' data-bs-target='#searchModal'><i className="bi bi-search fs-4 text-dark"></i></Link></li>
                            <li><Link to='/' data-bs-toggle='modal' data-bs-target='#signupModal'><i className="bi bi-person fs-4 text-dark"></i></Link></li>
                            <li>
                                <Link to='/wishlist' className='position-relative'>
                    ...
                    <div className="modal-content p-4">
                        <div className="modal-header border-0">
                            ...
                            <button
                                ...
                                aria-label='close'
                                onClick={() => localStorage.setItem('signupDismissed', true)}
                            ></button>
                        </div>
                ...
                <div className="modal-dialog modal-dialog-centered">
                    <div className="modal-content p-4">
                        ...
                        <div className="modal-body">
                            <form>
                                <div className="mb-3">
                                    <input type="text" className="form-control shadow-sm" placeholder='Search for Product' autoFocus onChange={handleChange} />
                                </div>
